...
void flags(int stageNumber){

    char* flag1 = ("hackmac{ch_33ky_che_eky_fla_ggy}\n");
    char* flag2 = ("hackmac{h@_ck3r_@13_rt}\n");
    char* flag3 = ("hackmac{y0u_@re_@_Hak_ker!}\n");
    char* flag4 = ("hackmac{plz_d0n+_h@ck_m3}\n");
    char* flag5 = ("hackmac{c0n_grat_ula+10n5}\n");

    switch(stageNumber){
        
        case 1:
            printf("\n\n");
            printf("Congratulations the flag is below: \n");
            printf("%s\n",flag1);
            printf("\n\n");
            break;
        ...
        default :
            printf("\n\n");
            printf("NAAAAAAAHHHHHH MATE INVALID ATTEMPT!!!");
            BOOOM();
            printf("\n\n");
            break;
    }
...
void secret_stage(){ //Secret Stage hidden in setup
    char* input;
    ...
    if (strings_not_equal(input,message)!= 0){
        ...
        printf("!!!BOMB HAS BEEN SUCESSFULLY DEFUSED!!!\n");
        flags(5);
        sleep(3);
        exit(18);
    } 
